...
import { directionsUrl, matchesMood, MOODS, photoUrl, placeSubtitle, placeUrl, ratingText, type ClientPlace } from '../lib/view.ts';
import { formatPrice } from '../lib/text.ts';
import { bindSaveButton, bindThemeToggle, readData, setTone, shuffle, withStatus } from './common.ts';
...
// Open places first (featured, then best rated, shuffled within ties), then places with unknown hours.
function buildQueue() {
  const now = vnNow();
  const list = withStatus(data.places.filter((p) => matchesMood(p, mood)), now);
  const rank = (p: ClientPlace) => (p.noiBat ? 10 : 0) + (p.wgoCham ?? 0);
  const open = shuffle(list.filter((x) => x.status.kind === 'open').map((x) => x.place)).sort((a, b) => rank(b) - rank(a));
...
function renderPick() {
  ...
  if (place.anh) img.src = photoUrl(place.anh);
