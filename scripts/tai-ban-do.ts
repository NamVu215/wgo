// npm run tai-ban-do [-- 20260914]
// Downloads the basemap for CITY.bounds from a Protomaps daily build (OpenStreetMap data)
// into public/nen-ban-do/, so the map needs no third-party tile server or API key.
import { mkdir, rename, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { PMTiles } from 'pmtiles';
import { CITY } from '../src/config.ts';
...
console.log(`Đã tải ${count} ô bản đồ (${(bytes / 1e6).toFixed(1)} MB) cho ${CITY.name}, cùng font chữ và biểu tượng.`);
